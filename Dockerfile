...
ADD https://api.github.com/repos/yuanxcoin/gyuanx-storage-server/git/refs/heads/master version.json
...
RUN cd gyuanx-storage-server \
    && mkdir -p build \
    && cd build \
    && cmake .. -DBOOST_ROOT=$BOOST_ROOT -Dsodium_USE_STATIC_LIBS=ON \
    && cmake --build . -- -j8

RUN gyuanx-storage-server/build/httpserver/gyuanx-storage --version 
