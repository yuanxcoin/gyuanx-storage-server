...
void serialize_message(std::string& res, const T& msg) {
    ...
    GYUANX_LOG(trace, "serialized message: {}", msg.data);
}
...
std::vector<message_t> deserialize_messages(const std::string& blob) {
    ...
    GYUANX_LOG(trace, "=== Deserializing ===");
    ...
    while (!slice.empty()) {
        ...
        /// Deserialize PK
        auto pk = deserialize_string(slice, gyuanx::get_user_pubkey_size());
        if (!pk) {
            GYUANX_LOG(debug, "Could not deserialize pk");
            return {};
        }
        ...
        if (!hash) {
            GYUANX_LOG(debug, "Could not deserialize hash");
            return {};
        }
        ...
        if (!data) {
            GYUANX_LOG(debug, "Could not deserialize data");
            return {};
        }
        ...
        if (!ttl) {
            GYUANX_LOG(debug, "Could not deserialize ttl");
            return {};
        }
        ...
        if (!timestamp) {
            GYUANX_LOG(debug, "Could not deserialize timestamp");
            return {};
        }
        ...
        if (!nonce) {
            GYUANX_LOG(debug, "Could not deserialize nonce");
            return {};
        }

        GYUANX_LOG(trace, "Deserialized data: {}", *data);

        GYUANX_LOG(trace, "pk: {}, msg: {}", *pk, *data);

        result.push_back({*pk, *data, *hash, *ttl, *timestamp, *nonce});
    }

    GYUANX_LOG(trace, "=== END ===");
