...
void Swarm::set_swarm_id(swarm_id_t sid) {
    ...
    if (sid == INVALID_SWARM_ID) {
        GYUANX_LOG(warn, "We are not currently an active Service Node");
    } else {

        if (cur_swarm_id_ == INVALID_SWARM_ID) {
            GYUANX_LOG(info, "EVENT: started SN in swarm: {}", sid);
        } else if (cur_swarm_id_ != sid) {
            GYUANX_LOG(info, "EVENT: got moved into a new swarm: {}", sid);
        }
    }
...
static all_swarms_t apply_ips(const all_swarms_t& swarms_to_keep,
    ...
    GYUANX_LOG(debug, "Updated {} entries from gyuanxd", updates_count);
    return result_swarms;
}

void Swarm::apply_swarm_changes(const all_swarms_t& new_swarms) {

    GYUANX_LOG(trace, "Applying swarm changes");
...
void Swarm::update_state(const all_swarms_t& swarms,
    ...
    if (active) {
        ...
        if (events.dissolved) {
            GYUANX_LOG(info, "EVENT: our old swarm got DISSOLVED!");
        }

        for (const sn_record_t& sn : events.new_snodes) {
            GYUANX_LOG(info, "EVENT: detected new SN: {}", sn);
        }

        for (swarm_id_t swarm : events.new_swarms) {
            GYUANX_LOG(info, "EVENT: detected a new swarm: {}", swarm);
        }
